...
MAIN_LOOP:
    ...
    BCF     LATB, RB0         ; Apaga el LED en RB0
    CALL    DELAY              ; Llama a la rutina de retardo
    GOTO    MAIN_LOOP          ; Repite el ciclo

DELAY:
    MOVLW   0xFF               ; Carga el registro W con 0xFF
    MOVWF   0x20               ; Mueve W al registro 0x20
    
    MOVLW   0xFF               ; Carga W con 255
    MOVWF   0x22               ; Segundo contador externo   

DELAY_LOOP:
    DECFSZ  0x22, F            ; Decrementa el registro 0x20, salta si es cero
    GOTO    DELAY_LOOP         ; Repite el bucle de retardo
    
    DECFSZ  0x21, F            ; Decrementa el primer contador
    GOTO    DELAY_LOOP         ; Si no es cero, repite
    
    RETURN                     ; Retorna de la rutina de retardo

    END                       ; Fin del programa

    ;se prende 1seg se apaga 1seg